...
class UserAddress(models.Model):
    ...
    create_time = models.DateTimeField(verbose_name='创建时间', help_text='创建时间', null=True, blank=True,
                                       auto_now_add=True)
    user = models.ForeignKey(to='WxUser', on_delete=models.CASCADE, verbose_name='用户', help_text='用户ID')
...
class UserProductCart(models.Model):
    ...
    user = models.ForeignKey(verbose_name='用户', help_text='用户ID', to='WxUser', on_delete=models.CASCADE)
    product = models.ForeignKey(to=Product, verbose_name='商品', help_text='商品ID', on_delete=models.CASCADE)
...
class UserOrder(models.Model):
    ...
    user = models.ForeignKey(to='WxUser', on_delete=models.CASCADE, help_text='用户ID', verbose_name='用户')
    status = models.SmallIntegerField(choices=status_choices, default=1, verbose_name='订单状态', help_text="订单状态(1, '等待付款'), (2, '等待发货'), (3, '运输中'), (4, '已签收'), (5, '已取消')")
